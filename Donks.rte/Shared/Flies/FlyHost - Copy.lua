...
function removeFly(self, flyType, i)
	table.remove(self.flyTable[flyType], i);
end

function spawnFly(self, flyType, spawnPos)
...
function fireBoolet(self, pos, angle)
...
function flyMotion(flyType, self)
...
function flySightCheck(self, fly)
...
function getAllWounds(self, woundTable)
...
function Update(self)
	if (self:IsDead() == false) then
		...
		local flyCheck = self:GetNumberValue("DonkFliesToAdd");
		if (flyCheck == 1) then
			for particle in MovableMan.Particles do
				if (particle.ClassName == "MOSRotating" and ToMOSRotating(particle):GetNumberValue("DonkTargetOwner") == self.UniqueID) then
					addFly(self, particle);
					ToMOSRotating(particle):SetNumberValue("DonkTargetOwner", 0);
				end
			end
			
			self:SetNumberValue("DonkFliesToAdd", 0);
		end
		...
		if (#actorTable > 0) then
			...
			for flyType = 1, #self.flyTable, 1 do
				while #self.flyTable[flyType] > 0 do
					...
					if (fly) then
						ToMOSRotating(fly):SetNumberValue("DonkTargetOwner", target.UniqueID);
						removeFly(self, flyType, 1);
					end
				end
			end
			self.flyTable = {{},{}};
			target:SetNumberValue("DonkFliesToAdd", 1);
		end
	end
end
