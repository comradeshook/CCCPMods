...
function Update(self)
	...
	if self:GetController():IsState(Controller.PRIMARY_ACTION) then
		...
		if self.shottimer:IsPastSimMS(1000) == true then
			--define what's going to be created
			if self.HFlipped == false then
				emitter = CreateAEmitter("Mini Slammer Fire Right")
				self.Vel.X = self.Vel.X - (4 * math.cos(self.RotAngle))
				self.Vel.Y = self.Vel.Y + (4 * math.sin(self.RotAngle))
			else
				emitter = CreateAEmitter("Mini Slammer Fire Left")
				self.Vel.X = self.Vel.X + (4 * math.cos(self.RotAngle))
				self.Vel.Y = self.Vel.Y - (4 * math.sin(self.RotAngle))
			end
