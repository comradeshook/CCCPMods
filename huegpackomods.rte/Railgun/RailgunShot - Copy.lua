...
function KillRay(self)
	...
	while struckMO ~= 255 do
		...
		if self.spawnEffects and MORoot.UniqueID ~= self.lastRootID then
			...
			local puff = CreateAEmitter("Railgun MO Hit Effect", "huegpackomods.rte")
			puff.Pos = lastHit
...
function Create(self)
	...
	self.barrierFound = SceneMan:CastStrengthRay(self.Pos, self.rangeVector, 350, self.hitVector, 1, 128, true)
	...
	while softTerrainCheck do
		failsafe = failsafe - 1
		local entryBoom = CreateAEmitter("Railgun Terrain Entry Blast", "huegpackomods.rte")
		...
		if terrainExitCheck and failsafe > 0 then
			local exitBoom = CreateAEmitter("Railgun Terrain Exit Blast", "huegpackomods.rte")
	...
	for i = 5, self.MOVector.Magnitude, 15 do
		...
		local effect = CreateAEmitter("Railgun Trail Effect", "huegpackomods.rte")
		effect.Pos = pos
		effect.RotAngle = self.RotAngle
		effect.HFlipped = self.HFlipped
		MovableMan:AddMO(effect)

		local lineDelete = CreateMOSRotating("Terrain Delete Line", "huegpackomods.rte")
		lineDelete.Pos = pos
	...
	if self.barrierFound then
		local blast = CreateAEmitter("Railgun Shatter Blast", "huegpackomods.rte")
		...
		local circleDelete = CreateMOSRotating("Terrain Delete Circle", "huegpackomods.rte")
